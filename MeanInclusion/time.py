...
def time_depth_methods(num_samples_list):
    ...
    for num_samples in num_samples_list:
        ...
        # 测试eID深度V2 (O(N^2)版本)
        print("  计算eID深度V2 (O(N^2)版本)...")
        start_time = time.time()
        eid_sorted_indices_v2, eid_depths_v2 = sorted_depth(contours_masks, depth="eid")
        eid_v2_time = time.time() - start_time
        
        # 测试CBD深度
        print("  计算CBD深度...")
        start_time = time.time()
        cbd_sorted_indices, cbd_depths = sorted_depth(contours_masks, depth="cbd")
        cbd_time = time.time() - start_time
        ...
        results['eid_v2_time'].append(eid_v2_time)
        results['cbd_time'].append(cbd_time)
        results['inclusion_time'].append(inclusion_time)
        
        # 打印当前结果
        print(f"  eID V1时间: {eid_v1_time:.4f}秒")
        print(f"  eID V2时间: {eid_v2_time:.4f}秒")
        print(f"  CBD时间: {cbd_time:.4f}秒")
        print(f"  包含分数时间: {inclusion_time:.4f}秒")
        print(f"  eID V1比V2快: {eid_v2_time/eid_v1_time:.2f}倍")
...
plt.plot(df_results['num_samples'], df_results['eid_v2_time'], 'bs-', label='eID V2 (O(N²))', linewidth=2, markersize=6)
plt.plot(df_results['num_samples'], df_results['cbd_time'], 'g^-', label='CBD', linewidth=2, markersize=6)
...
# 计算和显示性能比较
print(f"\n性能比较 (相对于eID V1的倍数):")
print(f"{'样本数':>8} | {'eID V2/V1':>10} | {'CBD/V1':>8} | {'包含/V1':>8}")
print("-" * 42)
for i, num_samples in enumerate(df_results['num_samples']):
    eid_v2_ratio = df_results['eid_v2_time'][i] / df_results['eid_v1_time'][i]
    cbd_ratio = df_results['cbd_time'][i] / df_results['eid_v1_time'][i]
    inclusion_ratio = df_results['inclusion_time'][i] / df_results['eid_v1_time'][i]
    print(f"{num_samples:>8} | {eid_v2_ratio:>10.2f} | {cbd_ratio:>8.2f} | {inclusion_ratio:>8.2f}")

# 额外分析两种eID算法的复杂度差异
print(f"\neID算法复杂度分析:")
print(f"{'样本数':>8} | {'V1时间':>8} | {'V2时间':>8} | {'复杂度比':>8}")
print("-" * 40)
for i, num_samples in enumerate(df_results['num_samples']):
    v1_time = df_results['eid_v1_time'][i]
    v2_time = df_results['eid_v2_time'][i]
    complexity_ratio = v2_time / v1_time
    print(f"{num_samples:>8} | {v1_time:>8.4f} | {v2_time:>8.4f} | {complexity_ratio:>8.2f}")