...
def time_depth_methods(grid_sizes_list, num_samples=100):
    """测试不同网格尺寸下各种深度方法的计算时间"""
    ...
    for grid_size in grid_sizes_list:
        ...
        print(f"  实际处理网格大小: {grid_size}x{grid_size}")
    
    return results

# 测试不同的网格尺寸（固定样本数量为100）
grid_sizes = [100, 500, 1000, 2000, 3000, 4000, 5000]
print("开始测试不同网格尺寸下的深度计算时间（固定样本数量为100）...")

# 运行测试
timing_results = time_depth_methods(grid_sizes)
...
# 保存到CSV文件
df_results.to_csv('depth_timing_results_by_grid_size.csv', index=False)
print(f"\n结果已保存到 depth_timing_results_by_grid_size.csv")

# 可视化时间对比
plt.figure(figsize=(12, 8))

plt.plot(df_results['grid_size'], df_results['eid_v1_time'], 'ro-', label='eID V1 (O(N))', linewidth=2, markersize=6)
plt.plot(df_results['grid_size'], df_results['inclusion_time'], 'md-', label='Inclusion Score', linewidth=2, markersize=6)

plt.xlabel('Grid Size (num_rows = num_cols)', fontsize=12)
plt.ylabel('Computation Time (seconds)', fontsize=12)
plt.title('Computation Time vs Grid Size for Different Depth Methods (100 samples)', fontsize=14)
...
# 计算和显示性能比较
print(f"\n性能比较 (相对于eID V1的倍数):")
print(f"{'网格尺寸':>8} | {'包含/V1':>8}")
print("-" * 20)
for i, grid_size in enumerate(df_results['grid_size']):
    inclusion_ratio = df_results['inclusion_time'][i] / df_results['eid_v1_time'][i]
    print(f"{grid_size:>8} | {inclusion_ratio:>8.2f}")

# eID V1和包含分数深度的性能对比分析
print(f"\neID V1和包含分数深度性能对比:")
print(f"{'网格尺寸':>8} | {'eID V1时间':>10} | {'包含时间':>10} | {'性能比':>8}")
print("-" * 44)
for i, grid_size in enumerate(df_results['grid_size']):
    v1_time = df_results['eid_v1_time'][i]
    inclusion_time = df_results['inclusion_time'][i]
    performance_ratio = inclusion_time / v1_time
    print(f"{grid_size:>8} | {v1_time:>10.4f} | {inclusion_time:>10.4f} | {performance_ratio:>8.2f}")