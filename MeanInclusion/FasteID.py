...
print("深度计算方法时间对比总结:")
print("="*60)
print(f"{'方法':^15} | {'时间(秒)':^12} | {'相对于eID的倍数':^15}")
print("-" * 60)
print(f"{'eID':^15} | {eid_time:^12.4f} | {'1.00':^15}")
print(f"{'CBD':^15} | {cbd_time:^12.4f} | {cbd_time/eid_time:^15.2f}")
print(f"{'PCA+ED':^15} | {pca_time:^12.4f} | {pca_time/eid_time:^15.2f}")
print(f"{'包含分数':^15} | {inclusion_time:^12.4f} | {inclusion_time/eid_time:^15.2f}")
print("="*60)
print(f"包含分数方法比eID快 {eid_time/inclusion_time:.1f} 倍")
print("="*60)

# 可视化时间对比
fig_time, ax_time = plt.subplots(1, 1, figsize=(10, 6))
methods_time = ['eID', 'CBD', 'PCA+ED', 'Inclusion Score']
times = [eid_time, cbd_time, pca_time, inclusion_time]
colors = ['red', 'orange', 'blue', 'green']

bars = ax_time.bar(methods_time, times, color=colors, alpha=0.7)
ax_time.set_ylabel('Time (seconds)', fontsize=12)
ax_time.set_title('Computation Time Comparison of Different Depth Methods', fontsize=14)
