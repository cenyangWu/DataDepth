...
def main():
    print("=== ET文件深度分析工具（numba.cuda 版本） ===")
    ...
    # 设置输出目录路径
    output_dir = r"D:\MyProjects\DataDepthData\ET_DepthAnalysis"

    print(f"数据源: soft_masks中的ET文件")
    print(f"输出目录: {output_dir}")

    # 1. 加载ET文件作为mask数据
    t0 = time.time()
    masks_data, mask_names, affine, header = load_ET_files_as_masks()
    ...
    # 1.5. 将所有输入数据以0.5为阈值转换为二值化数据
    print(f"\n=== 对所有输入数据进行0.5阈值二值化处理 ===")
    print(f"原始数据范围: {np.min(masks_data):.4f} - {np.max(masks_data):.4f}")
    print(f"原始数据形状: {masks_data.shape}")
    ...
    print(f"二值化后数据范围: {np.min(binary_masks_data):.4f} - {np.max(binary_masks_data):.4f}")
    
    # 统计二值化前后的激活体素数量
    original_voxels = np.count_nonzero(masks_data)
    binary_voxels = np.count_nonzero(binary_masks_data)
    total_voxels = masks_data.size
    
    print(f"总体素数: {total_voxels:,}")
    print(f"原始数据激活体素总数: {original_voxels:,} ({original_voxels/total_voxels*100:.2f}%)")
    print(f"二值化后激活体素总数: {binary_voxels:,} ({binary_voxels/total_voxels*100:.2f}%)")
    if original_voxels > 0:
        print(f"二值化保留的激活体素比例: {binary_voxels/original_voxels*100:.2f}%")
    
    # 使用二值化数据进行后续的深度计算
    masks_data = binary_masks_data
    print(f"已将二值化数据用于后续深度计算")
    print("="*60)
...
def load_ET_files_as_masks():
    """
    加载ET文件并转换为与load_all_masks相同的格式
    返回: masks_data, mask_names, affine, header
    """
    print(f"\n=== 开始加载ET文件作为Mask数据 ===")
    start_time = time.time()
    ...
    # 首先提取ET文件
    et_data = load_ET_files_as_arrays()
    
    if not et_data:
        print("没有找到任何ET文件")
        return None, None, None, None
    
    print(f"找到 {len(et_data)} 个ET文件")
    
    # 获取第一个文件的信息作为参考
    first_folder = list(et_data.keys())[0]
    first_data_info = et_data[first_folder]
    mask_shape = first_data_info['shape']
    affine = first_data_info['affine']
    header = first_data_info['header']
    
    print(f"ET文件形状: {mask_shape}")
    print(f"数据类型: {first_data_info['dtype']}")
    
    # 创建masks_data数组
    masks_data = np.zeros((len(et_data),) + mask_shape, dtype=np.float32)
    mask_names = []
    
    successful_loads = 0
    for i, (folder_name, data_info) in enumerate(et_data.items()):
        try:
            print(f"正在处理 ({i+1}/{len(et_data)}): {folder_name}")
            data = data_info['data']
            ...
            masks_data[successful_loads] = data_f32
            mask_names.append(f"{folder_name}_ET")
            successful_loads += 1
            voxel_count = np.count_nonzero(data_f32)
            print(f"  激活体素数: {voxel_count:,}")
    ...
    if successful_loads < len(et_data):
        masks_data = masks_data[:successful_loads]
        print(f"实际成功处理: {successful_loads} 个文件")
    
    end_time = time.time()
    print(f"ET文件加载完成，耗时: {end_time - start_time:.2f}秒")
    print(f"最终数据形状: {masks_data.shape}")
    print(f"总激活体素: {np.count_nonzero(masks_data):,}")
    
    return masks_data, mask_names, affine, header

def extract_ET_files_from_soft_masks():
    """
    从D:\MyProjects\DataDepthData\soft_masks中的31个文件夹提取包含ET的nii.gz文件
    """
    ...
    for folder_name in sorted(os.listdir(source_dir)):
        ...
        if os.path.isdir(folder_path):
            ...
            # 查找文件夹中包含'ET'的nii.gz文件
            et_pattern = os.path.join(folder_path, "*ET*.nii.gz")
            et_files_in_folder = glob.glob(et_pattern)
            
            for et_file in et_files_in_folder:
                et_files.append(et_file)
                et_file_count += 1
                file_name = os.path.basename(et_file)
                file_size = os.path.getsize(et_file) / (1024 * 1024)  # 转换为MB
                print(f"  找到ET文件: {file_name} (大小: {file_size:.2f} MB)")
    
    print("-" * 60)
    print(f"扫描完成！")
    print(f"共扫描了 {folder_count} 个文件夹")
    print(f"找到了 {et_file_count} 个ET文件")
    
    # 显示所有ET文件的详细信息
    print("\n" + "=" * 80)
    print("所有包含ET的nii.gz文件列表:")
    print("=" * 80)
    
    for i, file_path in enumerate(et_files, 1):
    ...
    print("=" * 80)
    print(f"总计: {len(et_files)} 个ET文件")
    
    return et_files

def load_ET_files_as_arrays(et_files_list=None):
    """
    将ET文件加载为numpy数组，便于后续处理
    
    Args:
        et_files_list: ET文件路径列表，如果为None则自动提取
    
    Returns:
        dict: 包含文件夹名作为key，nii数据作为value的字典
    """
    if et_files_list is None:
        et_files_list = extract_ET_files_from_soft_masks()
    
    if not et_files_list:
        print("没有找到ET文件！")
        return {}
    ...
    for i, file_path in enumerate(et_files_list, 1):
        folder_name = os.path.basename(os.path.dirname(file_path))
        file_name = os.path.basename(file_path)
        
        try:
            print(f"加载 {i}/{len(et_files_list)}: {folder_name}/{file_name}")
    ...
    print(f"\n成功加载了 {len(et_data)} 个ET文件")
    print("=" * 60)
    
    # 显示加载摘要
    print("加载摘要:")
    total_memory = 0
    for folder_name, data_info in et_data.items():
